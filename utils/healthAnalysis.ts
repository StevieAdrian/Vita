...
export function calculateHealthStats(entries: DiaryEntry[]): HealthStatsSummary {
    const uniqueDates = new Set(entries.map(e => ensureDate(e.date).toDateString()));
    const trackingDays = uniqueDates.size;

    const bpValues = entries.filter(e => typeof e.systolic === "number" && !Number.isNaN(e.systolic) && typeof e.diastolic === "number" && !Number.isNaN(e.diastolic));
    const okBP = bpValues.filter(
        e => (e.systolic < 120 && e.diastolic < 80) || (e.systolic >= 120 && e.systolic <= 129 && e.diastolic < 80)
    ).length;
    const bpPct = bpValues.length > 0 ? Math.round((okBP / bpValues.length) * 100) : null;
    ...
    const hrPct = hrValues.length > 0 ? Math.round((okHR / hrValues.length) * 100) : null;
